...
pub async fn get_anthropic_models(api_key: Option<String>) -> Result<Vec<AnthropicModel>, String> {
    ...
    let api_key = match api_key {
        Some(key) if !key.trim().is_empty() => {
            match crate::database::repositories::setting::normalize_summary_api_key(&key) {
                Ok(key) => key,
                Err(_) => return Ok(get_fallback_models()),
            }
        }
        _ => {
    ...
    let mut api_key_header = match reqwest::header::HeaderValue::from_bytes(api_key.as_bytes()) {
        Ok(value) => value,
        Err(_) => return Ok(get_fallback_models()),
    };
