...
/// Creates structure: base_path/MeetingName_YYYY-MM-DD_HH-MM-SS_SessionId/
...
pub fn create_meeting_folder(
    ...
    let session_suffix = Uuid::new_v4().simple().to_string();
    let folder_name = format!("{}_{}_{}", sanitized_name, timestamp, &session_suffix[..8]);
    let meeting_folder = base_path.join(folder_name);

    // The random suffix plus exclusive creation prevents two sessions with the
    // same display name from ever reusing audio, metadata, or checkpoints.
...
mod meeting_folder_tests {
    ...
    #[test]
    fn repeated_display_names_create_exclusive_session_folders() {
        let base = tempfile::tempdir().expect("temporary recordings root");
        let base_path = base.path().to_path_buf();

        let first =
            create_meeting_folder(&base_path, "Design review", true).expect("first meeting folder");
        let second = create_meeting_folder(&base_path, "Design review", true)
            .expect("second meeting folder");
