...
fn convert_to_wav_with_ffmpeg(
    ...
    // The portable launcher and Rust startup redirect the process temp
    // directory to portable/app-data/temp. Never create a potentially large
    // decoded WAV beside a source video that may live on the system drive.
    let process_temp_dir = std::env::temp_dir();
    std::fs::create_dir_all(&process_temp_dir).map_err(|e| {
        anyhow!(
            "Failed to prepare temporary audio directory {}: {}",
            process_temp_dir.display(),
            e
        )
    })?;
