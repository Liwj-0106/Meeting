...
pub async fn list_audio_devices() -> Result<Vec<AudioDevice>> {
    tokio::task::spawn_blocking(list_audio_devices_blocking)
        .await
        .map_err(|error| anyhow!("Audio device enumeration task failed: {error}"))?
}

/// CPAL device enumeration is synchronous and some Windows drivers can block.
/// Keep it off Tokio workers so stop/reconnect timeouts remain responsive.
