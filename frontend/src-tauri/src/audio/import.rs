...
async fn create_meeting_with_transcripts(
    ...
    let events = segments
        ...
        .enumerate()
        .map(|(index, segment)| {
            create_import_final_event(
                &meeting_id,
                index,
                segment,
                confidences.get(index).copied(),
                language,
                asr_provider,
                asr_model,
            )
    ...
    // Store the final ASR results through the canonical event repository so
    // the immutable history and backwards-compatible materialized table are
    // created together in this transaction.
    for event in &events {
        TranscriptEventsRepository::upsert_revision_in_transaction(&mut tx, event)
            .await
            .map_err(|e| anyhow!("Failed to insert transcript event: {}", e))?;
    }
...
/// Convert one imported batch segment into a canonical, replayable final event.
/// The identities are deterministic for a meeting and segment order, making
/// retries/idempotency checks independent of the random legacy segment ID.
fn create_import_final_event(
    meeting_id: &str,
    index: usize,
    ...
    let session_id = format!("import-session::{meeting_id}");
    let utterance_id = format!("import-utterance::{meeting_id}::{sequence_id:08}");
    let event_id = format!("import-event::{meeting_id}::{sequence_id:08}::revision-0");
    ...
    Ok(StoredTranscriptEvent {
        ...
        audio_source: Some("imported".to_string()),
        speaker_id: None,
...
mod tests {
    ...
    #[test]
    fn import_final_event_has_stable_canonical_identity_and_provenance() {
        let segment = TranscriptSegment {
            id: "legacy-random-id".to_string(),
            text: "hello".to_string(),
            timestamp: "2026-09-01T12:00:00Z".to_string(),
            audio_start_time: Some(1.25),
            audio_end_time: Some(2.5),
            duration: Some(1.25),
            ..Default::default()
        };

        let first = create_import_final_event(
            "meeting-test",
            3,
            &segment,
            Some(0.92),
        ...
        .expect("create event");
        let replay = create_import_final_event(
            "meeting-test",
            3,
            &segment,
            Some(0.92),
        ...
        assert_eq!(first.utterance_id, replay.utterance_id);
        assert_eq!(first.session_id, replay.session_id);
        assert_eq!(first.revision, 0);
        assert_eq!(first.event_kind, "final");
        assert!(first.is_stable);
        assert_eq!(first.start_ms, Some(1_250));
        assert_eq!(first.end_ms, Some(2_500));
        assert_eq!(first.audio_source.as_deref(), Some("imported"));
        assert_eq!(first.language.as_deref(), Some("ja"));
        assert_eq!(first.asr_provider.as_deref(), Some("whisper"));
        assert_eq!(first.asr_model.as_deref(), Some("small-q5_1"));
        assert!((first.asr_confidence.expect("confidence") - 0.92).abs() < 1e-6);
    }
