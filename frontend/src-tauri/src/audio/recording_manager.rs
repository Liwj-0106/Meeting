...
use super::devices::{list_audio_devices, AudioDevice, DeviceType as AudioDeviceType};
...
use super::stream::{AudioStreamManager, AudioStreamStartupFailure};
use super::synchronizer::CaptureTopology;
use super::transcription::streaming::StreamingAsrCommand;

fn aggregate_operation_errors(operation: &str, errors: Vec<String>) -> Result<()> {
    if errors.is_empty() {
        Ok(())
    } else {
        Err(anyhow::anyhow!("{}: {}", operation, errors.join("; ")))
    }
}
...
impl RecordingManager {
    ...
    /// Attach a bounded streaming ASR actor to the next recording session.
    /// Audio delivery remains best-effort and never blocks the recorder.
    pub fn set_streaming_asr_sender(&mut self, sender: mpsc::Sender<StreamingAsrCommand>) {
        self.pipeline_manager.set_streaming_asr_sender(sender);
    }
    ...
    pub async fn start_recording(
        ...
        // Queue capture callbacks before hardware is opened. We intentionally
        // discover the *actual* successful topology before constructing the
        // synchronizer and track savers; an optional route that failed to open
        // must not become a fake all-silence raw track.
        if let Err(error) = self
            .pipeline_manager
            .prepare_capture_channel(self.state.clone())
        {
        ...
        let actual_microphone = self.state.get_microphone_device();
        let actual_system = self.state.get_system_device();
        let capture_topology =
            CaptureTopology::new(actual_microphone.is_some(), actual_system.is_some());

        let recording_sender = match self
            .recording_saver
            .start_accumulation(auto_save, capture_topology)
        {
            Ok(sender) => sender,
            Err(start_error) => {
                self.state.stop_recording();
                if let Err(error) = self.stream_manager.stop_streams() {
                    warn!(
                        "Failed to stop streams after saver startup error: {}",
                        error
                    );
                }
                if let Err(error) = self.pipeline_manager.stop().await {
                    warn!(
                        "Failed to stop capture ingress after saver startup error: {}",
                        error
                    );
                }
                let rollback_warning = self
                    .recording_saver
                    .rollback_failed_start_after_ingress_closed(&start_error.to_string())
                    .await
                    .err();
                return Err(match rollback_warning {
                    Some(warning) => anyhow::anyhow!(
                        "Failed to start recording persistence: {}; rollback warning: {}",
                        start_error,
                        warning
                    ),
                    None => start_error,
        ...
        let (mic_name, mic_kind) = if let Some(ref mic) = actual_microphone {
            let device_kind =
        ...
        let (sys_name, sys_kind) = if let Some(ref sys) = actual_system {
            let device_kind =
        ...
        self.recording_saver.set_device_info(
            actual_microphone.as_ref().map(|d| d.name.clone()),
            actual_system.as_ref().map(|d| d.name.clone()),
        );
        ...
        if let Err(start_error) = self.pipeline_manager.start(
            ...
            // `AudioPipelineManager::start` owns and drops recording_sender on
            // error. Close every capture ingress before awaiting the saver so
            // its receiver drains to EOF instead of hanging or being aborted.
            self.state.stop_recording();
            if let Err(error) = self.stream_manager.stop_streams() {
                warn!(
                    "Failed to stop streams after pipeline startup error: {}",
                    error
                );
            }
            if let Err(error) = self.pipeline_manager.stop().await {
                warn!("Failed to stop pipeline after startup error: {}", error);
            }
            let rollback_warning = self
                .recording_saver
                .rollback_failed_start_after_ingress_closed(&start_error.to_string())
            ...
            return Err(match rollback_warning {
                Some(warning) => anyhow::anyhow!(
                    "Failed to start audio pipeline: {}; rollback warning: {}",
                    start_error,
                    warning
                ),
                None => start_error,
        ...
        if let Some(ref mut monitor) = self.device_monitor {
            if let Err(e) = monitor.start_monitoring(actual_microphone, actual_system) {
    ...
    pub async fn stop_streams_only(&mut self) -> Result<()> {
        ...
        if let Err(e) = self.stream_manager.stop_streams() {
            ...
            shutdown_errors.push(format!("audio streams: {e}"));
        }

        // Stop audio pipeline
        if let Err(e) = self.pipeline_manager.stop().await {
            error!("Error stopping audio pipeline: {}", e);
            shutdown_errors.push(format!("audio pipeline: {e}"));
        }

        let result =
            aggregate_operation_errors("Recording stream shutdown failed", shutdown_errors);
        if result.is_ok() {
    ...
    pub async fn stop_streams_and_force_flush(&mut self) -> Result<()> {
        ...
        if let Err(e) = self.pipeline_manager.force_flush_and_stop().await {
            ...
            shutdown_errors.push(format!("audio pipeline flush: {e}"));
        }

        let result = aggregate_operation_errors(
            "Recording stream shutdown with final drain failed",
            shutdown_errors,
        );
        if result.is_ok() {
    ...
    pub async fn save_recording_only<R: tauri::Runtime>(
        ...
        // Save the recording with actual duration. Keep the result until after
        // state cleanup so both success and failure release device/session
        // references. RecordingSaver retains recoverable checkpoints whenever
        // it cannot prove final persistence completed.
        let save_result = self
            .recording_saver
            .stop_and_save(app, recording_duration)
            .await;

        // Keep the canonical media frame and session metadata alive until the
        // saver has attempted to finalize its manifest. Device references were
        // already released by stop_recording().
        self.state.cleanup();
        ...
        match save_result {
            ...
            Err(e) => {
                ...
                Err(anyhow::anyhow!("Failed to save recording: {e}"))
            }
        }
    }
    ...
    pub async fn stop_recording<R: tauri::Runtime>(
        ...
        let mut shutdown_errors = Vec::new();
        if let Err(error) = self.stop_streams_only().await {
            shutdown_errors.push(error.to_string());
        }
        if let Err(error) = self.save_recording_only(app).await {
            shutdown_errors.push(error.to_string());
        }

        let result =
            aggregate_operation_errors("Recording manager shutdown failed", shutdown_errors);
        if result.is_ok() {
    ...
    /// Selected routes that failed to open while another route remained
    /// healthy enough for the session to continue.
    pub fn stream_startup_failures(&self) -> Vec<AudioStreamStartupFailure> {
        self.stream_manager.startup_failures().to_vec()
    }
    ...
    /// Return a transcript-only handle that remains valid while this manager is
    /// temporarily moved during asynchronous shutdown.
    pub fn transcript_persistence_sink(&self) -> super::recording_saver::TranscriptPersistenceSink {
        self.recording_saver.transcript_persistence_sink()
    }
    ...
    /// Get the complete revision stream for deterministic reload replay.
    pub fn get_transcript_events(&self) -> Vec<super::recording_saver::TranscriptSegment> {
        self.recording_saver.get_transcript_events()
    }
    ...
    /// Abort a recording that opened successfully but failed a required native
    /// startup invariant before any public `recording-started` boundary.
    pub async fn rollback_started_recording(&mut self, reason: &str) -> Result<()> {
        ...
        if let Err(error) = self
            .recording_saver
            .rollback_failed_start_after_ingress_closed(reason)
            .await
        {
            rollback_errors.push(error);
        }
        self.state.cleanup();
        aggregate_operation_errors("Recording startup rollback failed", rollback_errors)
    }
    ...
    pub async fn attempt_device_reconnect(
                ...
                device.name == device_name
                    && match &device_type {
                        DeviceMonitorType::Microphone => {
                            device.device_type == AudioDeviceType::Input
                        }
        ...
        if let Some(device) = device {
            ...
            match device_type {
                DeviceMonitorType::Microphone => {
                    self.stream_manager
                        .rebuild_microphone_stream(device_arc, None)
                        .await?;
                    info!("✅ Microphone reconnected successfully; system audio remained active");
                    Ok(true)
                }
                DeviceMonitorType::SystemAudio => {
                    self.stream_manager
                        .rebuild_system_audio_stream(device_arc, None)
                        .await?;
                    info!("✅ System audio reconnected successfully; microphone remained active");
                    Ok(true)
                }
            }
        } else {
    ...
    /// Handle a device disconnect event without interrupting the healthy route.
    pub async fn handle_device_disconnect(
        ...
        let (device, recording_device_type, stop_result) = match &device_type {
            DeviceMonitorType::Microphone => (
                self.state.get_microphone_device(),
                RecordingDeviceType::Microphone,
                self.stream_manager.stop_microphone_stream(),
            ),
            DeviceMonitorType::SystemAudio => (
                self.state.get_system_device(),
                RecordingDeviceType::System,
                self.stream_manager.stop_system_audio_stream(),
            ),
        };

        if let Err(error) = stop_result {
            error!(
                "Failed to release disconnected {:?} route '{}': {}",
                device_type, device_name, error
            );
        }

        // Reconnection status is informational; RecordingState::is_active does
        // not depend on it, so the healthy route keeps feeding the pipeline.
        if let Some(device) = device {
...
#[cfg(test)]
mod tests {
    use super::{aggregate_operation_errors, RecordingManager};

    #[test]
    fn recording_manager_is_send_without_an_unsafe_override() {
        fn assert_send<T: Send>() {}
        assert_send::<RecordingManager>();
    }

    #[test]
    fn stage2_shutdown_error_aggregation_preserves_every_component_failure() {
        assert!(aggregate_operation_errors("stop", Vec::new()).is_ok());

        let error = aggregate_operation_errors(
            "stop",
            vec![
                "audio streams: driver rejected stop".to_string(),
                "audio pipeline: worker panicked".to_string(),
            ],
        )
        .unwrap_err()
        .to_string();

        assert!(error.contains("audio streams: driver rejected stop"));
        assert!(error.contains("audio pipeline: worker panicked"));
    }
}
