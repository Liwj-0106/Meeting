...
fn normalize_audio_device_preference(value: Option<String>) -> Option<String> {
    value.and_then(|device| {
        let trimmed = device.trim();
        if trimmed.is_empty() {
            None
        } else if trimmed.eq_ignore_ascii_case(DISABLED_AUDIO_DEVICE) {
            Some(DISABLED_AUDIO_DEVICE.to_string())
        } else {
...
fn normalize_recording_preferences(preferences: &mut RecordingPreferences) {
    preferences.preferred_mic_device =
        normalize_audio_device_preference(preferences.preferred_mic_device.take());
    preferences.preferred_system_device =
        normalize_audio_device_preference(preferences.preferred_system_device.take());
}
...
fn validate_portable_recording_folder(path: &std::path::Path) -> Result<()> {
    if crate::storage::configured_data_dir().is_some() {
        crate::storage::validate_portable_storage_path(path, "portable recording folder")
...
pub fn get_default_recordings_folder() -> PathBuf {
    if let Some(data_dir) = crate::storage::configured_data_dir() {
        return data_dir.join("recordings");
    }
...
pub fn ensure_recordings_directory(path: &PathBuf) -> Result<()> {
    validate_portable_recording_folder(path)?;
    if !path.exists() {
        std::fs::create_dir_all(path)?;
        info!("Created recordings directory: {:?}", path);
    }
    // Validate the newly created directory as well. This resolves any parent
    // junction before recording files are opened.
    validate_portable_recording_folder(path)?;
    Ok(())
}
...
pub async fn load_recording_preferences<R: Runtime>(
    ...
    let store_path = crate::storage::store_path(app, "recording_preferences.json")?;
    ...
    if validate_portable_recording_folder(&prefs.save_folder).is_err() {
        let safe_folder = get_default_recordings_folder();
        warn!(
            "Stored recording folder is unsafe for portable mode; using configured default: {:?}",
            safe_folder
        );
        prefs.save_folder = safe_folder;
    }

    normalize_recording_preferences(&mut prefs);
...
pub async fn save_recording_preferences<R: Runtime>(
    ...
    validate_portable_recording_folder(&preferences.save_folder)?;
    // Create and revalidate the target before persisting the preference. If a
    // parent junction resolves onto C, the unsafe path must never become the
    // saved default for a later recording session.
...
pub async fn set_recording_preferences<R: Runtime>(
    ...
    normalize_recording_preferences(&mut preferences);
...
#[cfg(test)]
mod portable_recording_folder_tests {
    use super::{normalize_audio_device_preference, validate_portable_recording_folder};
    use std::path::Path;

    #[test]
    fn audio_device_preferences_have_one_canonical_disabled_sentinel() {
        assert_eq!(normalize_audio_device_preference(None), None);
        assert_eq!(
            normalize_audio_device_preference(Some("  ".to_string())),
            None
        );
        assert_eq!(
            normalize_audio_device_preference(Some(" DISABLED ".to_string())),
            Some("disabled".to_string())
        );
        assert_eq!(
            normalize_audio_device_preference(Some("  USB microphone (input)  ".to_string())),
            Some("USB microphone (input)".to_string())
        );
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn portable_path_validator_detects_windows_c_drive_case_insensitively() {
        // The configured-data guard is exercised by storage tests. This
        // helper remains a no-op outside portable mode, so validate the shared
        // path policy directly here.
        assert!(crate::storage::validate_portable_storage_path(
            Path::new(r"C:\Meetily\recordings"),
            "test recording folder"
        )
        .is_err());
        assert!(crate::storage::validate_portable_storage_path(
            Path::new(r"c:\Meetily\recordings"),
        ...
        .is_err());
        assert!(crate::storage::validate_portable_storage_path(
            Path::new(r"D:\Meetily\recordings"),
            "test recording folder"
        )
        .is_ok());

        let _ = validate_portable_recording_folder(Path::new(r"D:\Meetily\recordings"));
    }
}
