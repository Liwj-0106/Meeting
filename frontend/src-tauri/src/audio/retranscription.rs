...
use crate::database::repositories::transcript_event::TranscriptEventsRepository;
...
async fn run_retranscription<R: Runtime>(
    ...
    let mut transcript_confidences: Vec<f32> = Vec::new();
    ...
    for (i, segment) in processable_segments.iter().enumerate() {
        ...
        if !trimmed.is_empty() {
            ...
            transcript_confidences.push(conf);
    ...
    let asr_provider = if use_parakeet { "parakeet" } else { "whisper" };
    let asr_model = if let Some(engine) = parakeet_engine.as_ref() {
        engine.get_current_model().await.or_else(|| model.clone())
    } else if let Some(engine) = whisper_engine.as_ref() {
        engine.get_current_model().await.or_else(|| model.clone())
    } else {
        model.clone()
    };
    let events = segments
        ...
        .enumerate()
        .map(|(index, segment)| {
            create_retranscription_final_event(
                &meeting_id,
                index,
                segment,
                transcript_confidences.get(index).copied(),
                language.as_deref(),
                asr_provider,
                asr_model.as_deref(),
            )
    ...
    // Replace both event history and the materialized view in one transaction.
    // Retranscription deliberately starts a new canonical baseline rather than
    // preserving revisions produced by the previous ASR run.
    let pool = app_state.db_manager.pool();
    replace_meeting_transcript_events(pool, &meeting_id, &events).await?;
...
/// Replace a meeting's current transcript and immutable revision history as a
/// single unit. Any failure rolls back both deletes and every inserted event.
async fn replace_meeting_transcript_events(
    pool: &sqlx::SqlitePool,
    meeting_id: &str,
    events: &[StoredTranscriptEvent],
) -> Result<()> {
    if events.iter().any(|event| event.meeting_id != meeting_id) {
        return Err(anyhow!(
            "Retranscription event meeting ID does not match replacement target"
        ));
    }
    ...
    sqlx::query("DELETE FROM utterance_revisions WHERE meeting_id = ?")
        .bind(meeting_id)
        .execute(&mut *tx)
        .await
        .map_err(|e| anyhow!("Failed to delete existing transcript revisions: {}", e))?;
    ...
    for event in events {
        TranscriptEventsRepository::upsert_revision_in_transaction(&mut tx, event)
            .await
            .map_err(|e| anyhow!("Failed to insert retranscription event: {}", e))?;
    }

    tx.commit()
        .await
        .map_err(|e| anyhow!("Failed to commit transcript replacement: {}", e))?;
    Ok(())
}

fn create_retranscription_final_event(
    meeting_id: &str,
    index: usize,
    segment: &crate::api::TranscriptSegment,
    confidence: Option<f32>,
    language: Option<&str>,
    asr_provider: &str,
    asr_model: Option<&str>,
) -> Result<StoredTranscriptEvent> {
    let sequence_id = i64::try_from(index)
        .map_err(|_| anyhow!("Transcript sequence exceeds SQLite INTEGER range"))?;
    let session_id = format!("retranscription-session::{meeting_id}");
    let utterance_id = format!("retranscription-utterance::{meeting_id}::{sequence_id:08}");
    let event_id = format!("retranscription-event::{meeting_id}::{sequence_id:08}::revision-0");
    ...
    let created_at = chrono::DateTime::parse_from_rfc3339(&segment.timestamp)
    ...
    Ok(StoredTranscriptEvent {
        ...
        asr_provider: Some(asr_provider.to_string()),
        asr_model: asr_model.map(str::to_string),
        asr_confidence: confidence.map(f64::from),
        asr_latency_ms: None,
...
mod tests {
    ...
    #[tokio::test]
    async fn transcript_replacement_rebuilds_history_and_materialized_view_together() {
        ...
        sqlx::query("INSERT INTO meetings (id, title, created_at, updated_at) VALUES (?, ?, ?, ?)")
            .bind("meeting-retranscription-test")
            .bind("test")
            .bind("2026-09-01T12:00:00Z")
            .bind("2026-09-01T12:00:00Z")
            .execute(&pool)
            .await
            .expect("seed meeting");

        let old_segment = crate::api::TranscriptSegment {
            id: "old".to_string(),
        ...
        let old_event = create_retranscription_final_event(
            "meeting-retranscription-test",
            0,
            &old_segment,
            Some(0.5),
            Some("en"),
        ...
        .expect("create old event");
        TranscriptEventsRepository::upsert_revision(&pool, &old_event)
            .await
        ...
        let new_segments = [
            ...
            crate::api::TranscriptSegment {
                id: "new-2".to_string(),
                text: "new second".to_string(),
                timestamp: "2026-09-01T12:01:01Z".to_string(),
        ...
        replace_meeting_transcript_events(&pool, "meeting-retranscription-test", &replacements)
            .await
            .expect("replace transcripts");

        let history_count: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM utterance_revisions WHERE meeting_id = ?")
                .bind("meeting-retranscription-test")
                .fetch_one(&pool)
                .await
                .expect("count history");
        let latest: Vec<(String, String)> = sqlx::query_as(
            "SELECT transcript, asr_model FROM transcripts WHERE meeting_id = ? ORDER BY sequence_id",
        )
        .bind("meeting-retranscription-test")
        .fetch_all(&pool)
        .await
        .expect("read materialized transcripts");

        assert_eq!(history_count.0, 2);
        assert_eq!(latest.len(), 2);
        assert_eq!(
            latest[0],
            ("new first".to_string(), "new-model".to_string())
        );
        assert_eq!(
            latest[1],
            ("new second".to_string(), "new-model".to_string())
        );
    }
