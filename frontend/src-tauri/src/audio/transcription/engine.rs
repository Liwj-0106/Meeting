...
async fn load_safe_transcript_config<R: Runtime>(
    app: &AppHandle<R>,
) -> Result<crate::database::models::TranscriptConfigView, String> {
    let state = app.state::<crate::state::AppState>();
    crate::database::repositories::setting::SettingsRepository::get_transcript_config_view(
...
pub async fn validate_transcription_model_ready<R: Runtime>(
    ...
    let config = load_safe_transcript_config(app).await?;
    info!(
...
pub async fn get_or_init_transcription_engine<R: Runtime>(
    ...
    match config.provider.as_str() {
        ...
        other => Err(format!(
            "Provider '{other}' is not a local transcription engine"
        )),
    }
}
...
pub async fn get_or_init_whisper<R: Runtime>(
    ...
    if let Some(engine) = existing_engine {
        ...
        if engine.is_model_loaded().await {
            ...
            if config.provider != "localWhisper" {
                return Err(format!(
                    "Cannot initialize Whisper engine: config uses '{}' provider",
                    config.provider
                ));
            }
    ...
    let config = load_safe_transcript_config(app).await?;
    info!(
        "Got safe transcript config view - provider: {}, model: {}",
        config.provider, config.model
    );
    if config.provider != "localWhisper" {
        return Err(format!(
    ...
    let model_to_load = config.model;
