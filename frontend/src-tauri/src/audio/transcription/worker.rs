...
use super::event::{AudioSource, TranscriptChunkInput, TranscriptUpdate};
use super::provider::TranscriptionError;
use crate::audio::diarization::DiarizationSessionIngress;
...
pub fn start_transcription_task<R: Runtime>(
    ...
    tokio::spawn(async move {
        ...
        for worker_id in 0..NUM_WORKERS {
            ...
            let session_id = transcription_session_id.clone();
            let transcript_sink = transcript_sink.clone();
            let diarization_ingress = diarization_ingress.clone();
                ...
                loop {
                    ...
                    match chunk {
                        Some(chunk) => {
                            ...
                            let chunk_audio_source = match &chunk.device_type {
                                RecordingDeviceType::Microphone => AudioSource::Microphone,
                                RecordingDeviceType::System => AudioSource::System,
                                RecordingDeviceType::Mixed => AudioSource::Mixed,
                            };

                            if let Some(ingress) = &diarization_ingress {
                                // This bounded copy runs on the ASR worker, not
                                // the capture callback. Backpressure only drops
                                // the optional diarization window.
                                let _ = ingress.try_push_audio_chunk(chunk.clone());
                            }
                                ...
                                .await
                            {
                                Ok((transcript, confidence_opt, is_partial)) => {
                                    ...
                                    let confidence_threshold = match &engine_clone {
                                        // Local Whisper already passes VAD and text cleanup.
                                        // Do not silently drop short CJK captions based on a
                                    ...
                                    if !transcript.trim().is_empty() && meets_threshold {
                                        ...
                                        let mut event_input = TranscriptChunkInput::new(
                                            transcript,
                                            sequence_id,
                                            audio_start_time,
                                            audio_end_time,
                                        ...
                                        event_input.session_id = Some(session_id.clone());
                                        event_input.language =
                                            crate::get_language_preference_internal();
                                        event_input.model = if current_model == "unknown" {
                                            None
                                        } else {
                                            Some(current_model.clone())
                                        };
                                        event_input.trace_id =
                                            Some(format!("{}:chunk:{}", session_id, sequence_id));
                                        let update =
                                            TranscriptUpdate::from_legacy_chunk(event_input);

                                        // Persist trusted Rust output before it
                                        // becomes a renderer-facing event.
                                        transcript_sink.add_transcript_segment(
                                            TranscriptSegment::from(update.clone()),
                                        );
                                        if let Some(ingress) = &diarization_ingress {
