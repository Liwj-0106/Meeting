...
pub struct ContinuousVadProcessor {
    ...
    active_speech_start_ms: Option<f64>,
    /// Number of samples from the current Silero speech buffer that have already
    /// been emitted as committed live-caption chunks.
    emitted_speech_samples: usize,
    /// Optional upper bound for a live speech segment. Batch transcription leaves
    /// this disabled, while live captions use it to avoid waiting indefinitely for
    /// a pause in continuous speech.
    max_segment_samples: Option<usize>,
    /// Keep a small unsent tail so a natural stop never creates a tiny final
    /// chunk that Whisper cannot transcribe reliably.
    reserved_tail_samples: usize,
...
impl ContinuousVadProcessor {
    ...
    pub fn new_with_max_segment_duration(
        ...
        let max_segment_samples = max_segment_duration_ms
            ...
            .map(|duration_ms| {
                ((duration_ms as usize * VAD_SAMPLE_RATE as usize) / 1000).max(vad_chunk_size)
            });
        ...
        Ok(Self {
            ...
            in_speech: false,
            processed_samples: 0,
            active_speech_start_ms: None,
            emitted_speech_samples: 0,
            max_segment_samples,
            reserved_tail_samples: (250 * VAD_SAMPLE_RATE as usize) / 1000,
    ...
    pub fn flush(&mut self) -> Result<Vec<SpeechSegment>> {
        debug!("VAD flush: in_speech={}, active_speech_len={}, emitted_speech_samples={}, buffer_len={}, speech_segments_queued={}",
        ...
        // Emit only the uncommitted tail of an ongoing speech segment. Advancing
        // the cursor makes repeated flush calls idempotent without mutating
        // Silero's internal VAD state.
        if self.in_speech {
            let from = self.emitted_speech_samples;
            let remaining = {
                let active_speech = self.session.get_current_speech();
                active_speech.get(from..).unwrap_or_default().to_vec()
            };

            if !remaining.is_empty() {
                self.queue_segment(from, remaining, 0.8);
                self.emitted_speech_samples = self.session.current_speech_samples();
            }
        }
    ...
    fn process_chunk(&mut self, chunk: &[f32]) -> Result<()> {
        ...
        let current_speech_size = self
            .session
            .current_speech_samples()
            .saturating_sub(self.emitted_speech_samples);
        ...
        for transition in transitions {
            match transition {
                VadTransition::SpeechStart { timestamp_ms } => {
                    ...
                    self.in_speech = true;
                    // Silero timestamps are absolute within the VAD session.
                    self.active_speech_start_ms = Some(timestamp_ms as f64);
                ...
                VadTransition::SpeechEnd {
                    ...
                    // Silero returns the complete padded utterance. Live mode may
                    // already have committed one or more prefixes, so only queue
                    // the unseen suffix.
                    let full_speech = if !samples.is_empty() {
                        samples
                    } else {
                    ...
                    let from = self.emitted_speech_samples;
                    if from <= full_speech.len() {
                        let remaining = full_speech[from..].to_vec();
                        if !remaining.is_empty() {
                            self.active_speech_start_ms = Some(start_timestamp_ms as f64);
                            self.queue_segment(from, remaining, 0.9);
                        }
                    } else {
                        warn!(
                            "VAD: emitted cursor {} exceeded final segment length {}; refusing to duplicate audio",
                            from,
                            full_speech.len()
                        );
                    }

                    self.active_speech_start_ms = None;
                    self.emitted_speech_samples = 0;
                }
            }
        }

        // Commit bounded prefixes while speech remains active.
        if self.in_speech {
            self.emit_max_duration_segments();
        }
    ...
    /// Emit bounded chunks while Silero still considers speech active.
    ///
    /// Silero exposes its complete padded active-speech buffer. Keep that buffer
    /// intact and use a local cursor; consuming it with `take_until` would leave
    /// Silero's internal speech-start state pointing at deleted samples.
    fn emit_max_duration_segments(&mut self) {
        let Some(max_segment_samples) = self.max_segment_samples else {
            return;
        };

        while self.in_speech {
            let from = self.emitted_speech_samples;
            let segment_samples = {
                let active_speech = self.session.get_current_speech();
                let available = active_speech.len().saturating_sub(from);
                if available < max_segment_samples + self.reserved_tail_samples {
                    break;
                }
                active_speech[from..from + max_segment_samples].to_vec()
            };

            self.queue_segment(from, segment_samples, 0.85);
            self.emitted_speech_samples += max_segment_samples;
        }
    }

    fn queue_segment(&mut self, from: usize, samples: Vec<f32>, confidence: f32) {
        const VAD_SAMPLE_RATE: f64 = 16_000.0;

        let Some(active_start_ms) = self.active_speech_start_ms else {
            warn!("VAD: cannot queue speech without an active start timestamp");
            return;
        };

        let start_timestamp_ms = active_start_ms + from as f64 * 1000.0 / VAD_SAMPLE_RATE;
        let end_timestamp_ms = start_timestamp_ms + samples.len() as f64 * 1000.0 / VAD_SAMPLE_RATE;

        info!(
            "VAD: Queued live-caption segment: {:.1}ms duration, {} samples",
            end_timestamp_ms - start_timestamp_ms,
            samples.len()
        );
...
mod tests {
    ...
    #[test]
    fn test_live_mode_bounds_continuous_speech_segments() {
        let audio = generate_test_audio_with_speech(8.0, 16000);
        // The synthetic harmonic signal is only intermittently considered speech
        // by Silero. A short test bound still exercises multiple forced commits,
        // cursor handling, and the natural SpeechEnd suffix path deterministically.
        let max_segment_duration_ms = 500;

        let mut unlimited = ContinuousVadProcessor::new(16000, 400)
            .expect("Failed to create unlimited VAD processor");
        let mut unlimited_segments = Vec::new();
        for chunk in audio.chunks(1_600) {
            unlimited_segments.extend(unlimited.process_audio(chunk).expect("Processing failed"));
        }
        unlimited_segments.extend(unlimited.flush().expect("Flush failed"));

        let mut processor = ContinuousVadProcessor::new_with_max_segment_duration(
            16000,
            400,
            Some(max_segment_duration_ms),
        )
        .expect("Failed to create live VAD processor");

        let mut all_segments = Vec::new();
        for chunk in audio.chunks(1_600) {
            all_segments.extend(processor.process_audio(chunk).expect("Processing failed"));
        }
        all_segments.extend(processor.flush().expect("Flush failed"));

        assert!(
            processor.flush().expect("Second flush failed").is_empty(),
            "Repeated flush must not duplicate the live-caption tail"
        );

        assert!(
            all_segments.len() >= 2,
            "Expected continuous speech to be emitted incrementally, found {} segment(s)",
            all_segments.len()
        );

        // Natural SpeechEnd padding may add up to 400 ms to the final tail.
        let maximum_expected_samples = ((max_segment_duration_ms + 450) as usize * 16000) / 1000;
        assert!(
            all_segments
                .iter()
                .all(|segment| segment.samples.len() <= maximum_expected_samples),
            "Expected live segments to stay bounded near {} ms: {:?}",
            max_segment_duration_ms,
            all_segments
                .iter()
        ...
        let unlimited_audio = unlimited_segments
            .iter()
            .flat_map(|segment| segment.samples.iter().copied())
            .collect::<Vec<_>>();
        let live_audio = all_segments
            .iter()
            .flat_map(|segment| segment.samples.iter().copied())
            .collect::<Vec<_>>();
        assert_eq!(
            live_audio, unlimited_audio,
            "Live segmentation must neither drop nor duplicate VAD speech samples"
        );

        for pair in all_segments.windows(2) {
            assert!(
                (pair[0].end_timestamp_ms - pair[1].start_timestamp_ms).abs() <= 0.0625,
                "Adjacent live chunks should have continuous timestamps: {:?}",
                pair
            );
        }
    }
