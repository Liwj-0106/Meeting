...
async fn delete_meeting_with_transaction(
    ...
    // 3. Delete immutable transcript revision history explicitly. SQLite
    // foreign-key cascading is also configured, but the explicit delete keeps
    // cleanup correct for legacy connections which may have foreign_keys off.
    sqlx::query("DELETE FROM utterance_revisions WHERE meeting_id = ?")
        .bind(meeting_id)
        .execute(&mut *transaction)
        .await?;

    // 4. Delete from the latest transcript materialization
