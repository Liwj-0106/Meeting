...
// Global language preference storage. Preserve the spoken language by default;
// translation is an explicit user choice.
...
) -> Result<audio::recording_commands::RecordingStartResult, String> {
    log_info!(
        "🔥 CALLED start_recording (meeting_name_present={})",
        meeting_name.is_some()
    );
    log_info!(
        "📋 Backend received recording parameters - mic_selected={}, system_selected={}, meeting_name_present={}",
...
async fn save_transcript(file_path: String, content: String) -> Result<(), String> {
    log_info!(
        "Saving transcript export (characters={})",
        content.chars().count()
    );
...
async fn start_recording_with_devices_and_meeting<R: Runtime>(
    ...
    log_info!(
        "🚀 CALLED start_recording_with_devices_and_meeting - mic_selected={}, system_selected={}, meeting_name_present={}",
        mic_device_name.is_some(),
        system_device_name.is_some(),
        meeting_name.is_some()
    );
    ...
    let recording_result = match (mic_device_name.clone(), system_device_name.clone()) {
        (None, None) => {
            log_info!(
                "No devices specified, starting with defaults (meeting_name_present={})",
                meeting_name.is_some()
            );
        ...
        _ => {
            log_info!(
                "Starting with specified devices: mic_selected={}, system_selected={}, meeting_name_present={}",
                mic_device_name.is_some(),
                system_device_name.is_some(),
                meeting_name.is_some()
            );
...
pub fn run() {
    ...
    storage::initialize_process_paths()
        .expect("Failed to initialize the configured Meetily data directory");
    ...
    builder
        ...
        .manage(summary::live::LiveSummaryRuntimeState::default())
        .manage(audio::transcription::translation_runtime::LiveTranslationRuntimeState::default())
        .setup(|_app| {
            log::info!("Application setup complete");

            if let Err(e) = caption_overlay::initialize(_app.handle()) {
                log::error!("Failed to initialize caption overlay settings: {}", e);
            }
            ...
            // Repair a process exit between the atomic meeting/session commit
            // and the separate, idempotent translation promotion transaction.
            if let Some(state) = _app.try_state::<state::AppState>() {
                let pool = state.db_manager.pool().clone();
                if let Err(error) = tauri::async_runtime::block_on(async move {
                    audio::transcription::translation_runtime::reconcile_bound_staged_translations(
                        &pool,
                    )
                    .await
                }) {
                    log::warn!(
                        "Failed to reconcile durable live translations at startup: {}",
                        error
                    );
                }
            }
            ...
            storage::remove_empty_default_local_data_dir(_app.handle());

            Ok(())
        })
        .on_window_event(|window, event| {
            if let tauri::WindowEvent::Moved(position) = event {
                if window.label() == caption_overlay::CAPTION_OVERLAY_LABEL {
                    caption_overlay::handle_overlay_moved(window.app_handle().clone(), *position);
                }
            }
            if let tauri::WindowEvent::CloseRequested { api, .. } = event {
                ...
                } else if window.label() == caption_overlay::CAPTION_OVERLAY_LABEL {
                    api.prevent_close();
                    if let Err(e) = caption_overlay::set_caption_overlay_visible(
                        window.app_handle().clone(),
                        false,
                    ) {
                        log::error!("Failed to hide caption overlay on close request: {}", e);
                    } else {
                        log::info!("Caption overlay hidden on close request");
                    }
                } else if window.label() == caption_overlay::CAPTION_SETTINGS_LABEL {
                    api.prevent_close();
                    if let Err(e) = window.hide() {
                        log::error!("Failed to hide caption settings on close request: {}", e);
                    }
                }
            }
        })
        .invoke_handler(tauri::generate_handler![
            ...
            caption_overlay::set_caption_overlay_visible,
            caption_overlay::is_caption_overlay_visible,
            caption_overlay::set_caption_settings_visible,
            caption_overlay::get_caption_overlay_settings,
            caption_overlay::set_caption_overlay_settings,
            audio::transcription::translation_runtime::api_get_live_translation_settings,
            audio::transcription::translation_runtime::api_save_live_translation_settings,
            audio::transcription::translation_runtime::api_set_live_translation_api_key,
            audio::transcription::translation_runtime::api_clear_live_translation_api_key,
            audio::transcription::translation_runtime::api_queue_live_caption_translation,
            // Recording pause/resume commands
            audio::recording_commands::pause_recording,
            audio::recording_commands::resume_recording,
            audio::recording_commands::is_recording_paused,
            audio::recording_commands::get_recording_state,
            audio::recording_commands::get_audio_source_health_snapshot,
            audio::recording_commands::get_streaming_asr_health_snapshot,
            audio::recording_commands::get_meeting_folder_path,
            // Reload sync commands (retrieve transcript history and meeting name)
            audio::recording_commands::get_transcript_history,
            audio::recording_commands::get_recording_meeting_name,
            audio::recording_saver::read_recording_transcript_recovery,
            ...
            api::api_set_summary_api_key,
            api::api_delete_summary_api_key,
            // api::api_get_auto_generate_setting,
            // api::api_save_auto_generate_setting,
            api::api_get_transcript_config,
            api::api_save_transcript_config,
            api::api_set_transcript_api_key,
            ...
            // Revision-bound live summary commands. Provider responses stay
            // inside Rust and are never accepted from the WebView.
            summary::live::commands::api_live_summary_start_session,
            summary::live::commands::api_live_summary_request_final_reconcile,
            summary::live::commands::api_live_summary_get_current,
            summary::live::commands::api_live_summary_get_preferences,
            summary::live::commands::api_live_summary_save_preferences,
            summary::live::commands::api_live_summary_delete_custom_prompt,
            summary::live::commands::api_live_summary_prepare_recording,
            summary::live::commands::api_live_summary_list_recording_scopes,
            summary::live::commands::api_live_summary_issue_binding_ticket,
            // Template commands
