...
const ALLOW_EXTERNAL_OLLAMA_PULL_ENV: &str = "MEETILY_ALLOW_EXTERNAL_OLLAMA_PULL";

fn should_allow_ollama_pull(portable_mode: bool, explicit_opt_in: Option<&str>) -> bool {
    !portable_mode || explicit_opt_in.is_some_and(|value| value.trim() == "1")
}

fn ensure_ollama_pull_is_allowed() -> Result<(), String> {
    let portable_mode = crate::storage::configured_data_dir().is_some();
    let opt_in = std::env::var(ALLOW_EXTERNAL_OLLAMA_PULL_ENV).ok();
    if should_allow_ollama_pull(portable_mode, opt_in.as_deref()) {
        return Ok(());
    }

    Err(format!(
        "Portable mode blocked the Ollama model download because an already-running Ollama service may store models on the system drive. Configure that service's OLLAMA_MODELS directory on D:, restart it, then set {ALLOW_EXTERNAL_OLLAMA_PULL_ENV}=1 before launching Meetily."
    ))
}
...
#[cfg(test)]
mod portable_pull_tests {
    use super::should_allow_ollama_pull;

    #[test]
    fn portable_mode_requires_an_explicit_external_service_opt_in() {
        assert!(!should_allow_ollama_pull(true, None));
        assert!(!should_allow_ollama_pull(true, Some("0")));
        assert!(should_allow_ollama_pull(true, Some("1")));
        assert!(should_allow_ollama_pull(false, None));
    }
}
