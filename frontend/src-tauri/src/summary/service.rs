use crate::database::repositories::{
    ...
    setting::{SettingsRepository, SummaryApiKeyProvider},
...
impl SummaryService {
    ...
    pub async fn process_transcript_background<R: tauri::Runtime>(
        ...
        let key_provider = match &provider {
            LLMProvider::OpenAI => Some(SummaryApiKeyProvider::OpenAi),
            LLMProvider::Claude => Some(SummaryApiKeyProvider::Claude),
            LLMProvider::Groq => Some(SummaryApiKeyProvider::Groq),
            LLMProvider::OpenRouter => Some(SummaryApiKeyProvider::OpenRouter),
            LLMProvider::CustomOpenAI => Some(SummaryApiKeyProvider::CustomOpenAi),
            LLMProvider::Ollama | LLMProvider::BuiltInAI => None,
        };
        let api_key = if let Some(key_provider) = key_provider {
            match SettingsRepository::get_summary_api_key(&pool, key_provider).await {
                Ok(Some(key)) => Some(key),
                Ok(None) if provider == LLMProvider::CustomOpenAI => None,
                Ok(None) => {
                    Self::update_process_failed(
                        &pool,
                        &meeting_id,
                        "summary_api_key_missing|尚未配置所选总结服务的 API 密钥。",
                    )
                    .await;
                    return;
                }
                Err(error) => {
                    error!("Failed to retrieve summary credential: {}", error);
                    Self::update_process_failed(
                        &pool,
                        &meeting_id,
                        "summary_config_unavailable|无法读取总结服务配置，请稍后重试。",
                    )
                    .await;
                    return;
                }
        ...
        let (
            ...
            match SettingsRepository::get_custom_openai_config(&pool).await {
                ...
                Err(e) => {
                    error!("Failed to retrieve custom OpenAI public config: {}", e);
                    Self::update_process_failed(
                        &pool,
                        &meeting_id,
                        "summary_config_unavailable|无法读取自定义模型配置，请稍后重试。",
                    )
                    .await;
                    return;
                }
        ...
        let app_data_dir = crate::storage::app_data_dir(&_app).ok();
