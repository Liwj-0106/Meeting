...
fn get_custom_templates_dir() -> Option<PathBuf> {
    crate::storage::custom_templates_dir()
}

pub(crate) fn validate_template_id(template_id: &str) -> Result<(), String> {
    let bytes = template_id.as_bytes();
    let has_safe_shape = !bytes.is_empty()
        && bytes.len() <= MAX_TEMPLATE_ID_BYTES
    ...
    if has_safe_shape {
        ...
        Err("Invalid template identifier".to_string())
    }
}

/// Resolve an existing template while keeping symlinks and junctions inside
/// the configured template directory. Returning `None` intentionally makes an
/// unsafe or unreadable file indistinguishable from a missing template.
fn canonical_template_path(root: &std::path::Path, template_id: &str) -> Option<PathBuf> {
    let canonical_root = std::fs::canonicalize(root).ok()?;
    let candidate = canonical_root.join(format!("{}.json", template_id));
...
fn load_bundled_template(template_id: &str) -> Option<String> {
    ...
    let template_path = canonical_template_path(&bundled_dir, template_id)?;
...
fn load_custom_template(template_id: &str) -> Option<String> {
    ...
    let template_path = canonical_template_path(&custom_dir, template_id)?;
...
mod tests {
    ...
    #[test]
    fn rejects_template_identifiers_that_can_escape_the_template_directory() {
        for template_id in [
            "../secret",
            "nested/template",
            ".hidden",
            ...
            let error = get_template(template_id).unwrap_err();
            assert_eq!(error, "Invalid template identifier");
        }
    }

    #[test]
    fn resolves_existing_templates_only_inside_the_canonical_root() {
        let directory = tempfile::tempdir().unwrap();
        let template_path = directory.path().join("safe_template.json");
        fs::write(&template_path, "{}").unwrap();

        let resolved = canonical_template_path(directory.path(), "safe_template").unwrap();
        assert_eq!(resolved, fs::canonicalize(template_path).unwrap());
        assert!(canonical_template_path(directory.path(), "missing").is_none());
    }
