...
fn handle_menu_event<R: Runtime>(app: &AppHandle<R>, item_id: &str) {
    match item_id {
        ...
        "toggle_caption_overlay" => {
            if let Err(error) = crate::caption_overlay::toggle_caption_overlay(app) {
                log::error!("Failed to toggle caption overlay from tray: {}", error);
            }
        }
        "toggle_caption_mouse_passthrough" => {
            if let Err(error) = crate::caption_overlay::toggle_mouse_passthrough(app) {
                log::error!("Failed to toggle caption mouse passthrough: {}", error);
            }
        }
...
fn toggle_recording_handler<R: Runtime>(app: &AppHandle<R>) {
    ...
    tauri::async_runtime::spawn(async move {
        if crate::is_recording().await {
            ...
            let data_dir = match crate::storage::app_data_dir(&app_clone) {
                Ok(dir) => dir,
                Err(e) => {
...
fn build_menu<R: Runtime>(
    ...
    let caption_menu_label = match crate::caption_overlay::overlay_is_visible(app) {
        Ok(true) => "隐藏实时字幕",
        _ => "显示实时字幕",
    };
    let mouse_passthrough_menu_label = if crate::caption_overlay::mouse_passthrough_enabled() {
        "关闭字幕鼠标穿透"
    } else {
        "开启字幕鼠标穿透"
    };
    ...
    if !can_record {
        ...
        match state {
            ...
            RecordingState::Paused => {
                builder = builder
                    .item(&MenuItemBuilder::with_id("resume_recording", "▶ 继续录音").build(app)?)
                    .item(&MenuItemBuilder::with_id("stop_recording", "⏹ 停止录音").build(app)?);
            }
            RecordingState::Resuming => {
                builder = builder
                    .item(
                        &MenuItemBuilder::new("▶ 正在恢复…")
                            .enabled(false)
                            .build(app)?,
                    )
                    .item(&MenuItemBuilder::with_id("stop_recording", "⏹ 停止录音").build(app)?);
    ...
    builder
        ...
        .item(&MenuItemBuilder::with_id("toggle_caption_overlay", caption_menu_label).build(app)?)
        .item(
            &MenuItemBuilder::with_id(
                "toggle_caption_mouse_passthrough",
                mouse_passthrough_menu_label,
            )
            .build(app)?,
        )
        ...
        .item(&PredefinedMenuItem::separator(app)?)
        .item(&MenuItemBuilder::with_id("quit", "退出 Meetily").build(app)?)
        .build()
}
