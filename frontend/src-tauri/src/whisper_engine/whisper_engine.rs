...
impl WhisperEngine {
    ...
    pub fn new_with_models_dir(models_dir: Option<PathBuf>) -> Result<Self> {
        ...
        let models_dir = if let Some(dir) = models_dir {
            ...
            dir
        } else if let Some(data_dir) = crate::storage::configured_data_dir() {
            ...
            if cfg!(debug_assertions) {
                ...
                crate::storage::fallback_data_dir()
    ...
    fn clean_repetitive_text(text: &str) -> String {
        ...
        if Self::is_meaningless_output(text) {
            ...
            perf_debug!(
                "Detected meaningless output, returning empty (characters={})",
                text.chars().count()
            );
            return String::new();
        }
    ...
    pub async fn transcribe_audio_with_confidence(
        ...
        // This Whisper call receives a complete VAD/import chunk and returns once
        // inference for that chunk has finished. Chunk duration therefore says
        // nothing about revision stability: the live pipeline intentionally caps
        // chunks at a few seconds. Partial results are reserved for providers that
        // keep one utterance open and later emit a higher revision.
        ...
        let mut confidence_token_count = 0usize;
        ...
        for i in 0..num_segments {
            ...
            // Use Whisper's token probabilities. The previous text-length
            // heuristic assigned low confidence to valid short captions (and was
            // especially wrong for CJK text), causing the worker to discard them.
            if let Ok(token_count) = state.full_n_tokens(i) {
                for token_index in 0..token_count {
                    if let Ok(probability) = state.full_get_token_prob(i, token_index) {
                        if probability.is_finite() {
                            total_confidence += probability.clamp(0.0, 1.0);
        ...
        let avg_confidence = if confidence_token_count > 0 {
            total_confidence / confidence_token_count as f32
        } else {
            0.85
        };
