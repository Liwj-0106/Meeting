...
export function SettingsModals({
  ...
  const [deviceDraft, setDeviceDraft] = useState<SelectedDevices>(selectedDevices);
  const [isSavingDevices, setIsSavingDevices] = useState(false);

  useEffect(() => {
    if (modals.deviceSettings) {
      setDeviceDraft(selectedDevices);
    }
  }, [modals.deviceSettings, selectedDevices]);

  const handleSaveDevices = async () => {
    setIsSavingDevices(true);
    try {
      const saved = await saveSelectedDevices(deviceDraft);
      const normalizedDevices = selectedDevicesFromPreferences(saved);
      setDeviceDraft(normalizedDevices);
      ...
      toast.success("音频设备已保存", {
        description: `麦克风：${describeDevice(normalizedDevices.micDevice)}；系统音频：${describeDevice(normalizedDevices.systemDevice)}`,
      });
      onClose('deviceSettings');
    } catch (error) {
      if (error instanceof RecordingPreferencesSaveError && error.recoveredPreferences) {
        setDeviceDraft(selectedDevicesFromPreferences(error.recoveredPreferences));
      } else {
        setDeviceDraft(selectedDevices);
      }
      toast.error('音频设备保存失败，已恢复上次保存的值', {
    ...
    {modals.deviceSettings && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
          ...
          <DeviceSelection
            selectedDevices={deviceDraft}
            onDeviceChange={setDeviceDraft}
            disabled={isRecording || isSavingDevices}
          />

          <div className="mt-6 flex justify-end">
            <button
              onClick={() => void handleSaveDevices()}
              disabled={isRecording || isSavingDevices}
              className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isSavingDevices ? '保存中…' : '保存并应用'}
            </button>
          </div>
        </div>
      </div>
