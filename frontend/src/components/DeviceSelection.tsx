...
export { DISABLED_AUDIO_DEVICE } from '@/lib/recording-preferences';
...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const selectedMicUnavailable = isSelectedAudioDeviceUnavailable(
    selectedDevices.micDevice,
    devices,
    'Input',
  );
  const selectedSystemUnavailable = isSelectedAudioDeviceUnavailable(
  ...
  const getDeviceMetadata = (deviceName: string) => {
    ...
    if (deviceName === 'default') {
      category = 'default';
    } else if (deviceName === DISABLED_AUDIO_DEVICE) {
      category = 'disabled';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value={DISABLED_AUDIO_DEVICE}>不使用麦克风（仅系统音频）</SelectItem>
              {selectedMicUnavailable && selectedDevices.micDevice && (
                <SelectItem value={selectedDevices.micDevice} disabled>
                  {selectedDevices.micDevice}（当前不可用）
                </SelectItem>
              )}
              {inputDevices.map((device) => (
                <SelectItem
                  key={device.name}
                  value={audioDeviceOptionValue(device)}
                >
                  {device.name}
                </SelectItem>
              ))}
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">未发现麦克风；仍可选择“仅系统音频”。</p>
          )}
          {selectedMicUnavailable && (
            <p className="text-xs text-amber-700" role="status">
              已保存的麦克风当前不可用。刷新不会清除选择，请手动选择其他设备或系统默认。
            </p>
          )}
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="default">系统默认播放设备</SelectItem>
              <SelectItem value={DISABLED_AUDIO_DEVICE}>不使用系统音频（仅麦克风）</SelectItem>
              {selectedSystemUnavailable && selectedDevices.systemDevice && (
                <SelectItem value={selectedDevices.systemDevice} disabled>
                  {selectedDevices.systemDevice}（当前不可用）
                </SelectItem>
              )}
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">未发现系统音频播放设备。</p>
          )}
          {selectedSystemUnavailable && (
            <p className="text-xs text-amber-700" role="status">
              已保存的系统音频设备当前不可用。刷新不会清除选择，请手动选择其他设备或系统默认。
            </p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        ...
        <p>• <strong>系统音频：</strong>录制电脑播放的媒体与会议声音</p>
        <p>• <strong>视频字幕：</strong>选择“不使用麦克风”即可只捕获系统音频</p>
        {isMonitoring && (
          <p>• <strong>音量指示：</strong>绿色正常、黄色偏响、红色过响</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• 关闭某一路音频使用明确的“关闭”选项；“系统默认”会跟随 Windows 当前默认设备。</p>
        )}
      </div>
    </div>
  );
