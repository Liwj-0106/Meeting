...
import type {
  CustomOpenAIConfig,
  ModelConfig,
  SummaryApiKeyMutationResult,
  SummaryConnectionTestResult,
  SummaryKeyProvider,
...
export function ModelSettingsModal({
  ...
  // Secret drafts are intentionally short-lived and always start empty.
  ...
  const configuredKeyForProvider = (provider: ModelConfig['provider']): boolean => {
    switch (provider) {
      case 'openai': return modelConfig.apiKeyConfigured.openai;
      case 'claude': return modelConfig.apiKeyConfigured.claude;
      case 'groq': return modelConfig.apiKeyConfigured.groq;
      case 'openrouter': return modelConfig.apiKeyConfigured.openrouter;
      case 'custom-openai': return modelConfig.apiKeyConfigured.customOpenai;
      default: return false;
    }
  };
  const selectedProviderHasSavedKey = configuredKeyForProvider(modelConfig.provider);
  ...
  const isDoneDisabled =
    (requiresApiKey && !selectedProviderHasSavedKey && !apiKey.trim()) ||
          ...
          if (data.provider === 'custom-openai') {
            try {
              ...
              if (customConfig) {
                ...
                setModelConfig((previous: ModelConfig) => ({
                  ...previous,
  ...
  const loadOpenAIModels = async (key: string | null) => {
    ...
    try {
      ...
      console.error('Error loading OpenAI models.');
      setOpenaiModels([]); // Will use fallback via modelOptions
    } finally {
  ...
  const loadClaudeModels = async (key: string | null) => {
    ...
    try {
      ...
      console.error('Error loading Claude models.');
      setClaudeModels([]); // Will use fallback via modelOptions
    } finally {
  ...
  const loadGroqModels = async (key: string | null) => {
    ...
    try {
      ...
      console.error('Error loading Groq models.');
      setGroqModels([]); // Will use fallback via modelOptions
    } finally {
  ...
  const handleSave = async () => {
    let apiKeyConfigured = modelConfig.apiKeyConfigured;
    let hasApiKey = selectedProviderHasSavedKey;
    let customOpenAIHasApiKey = modelConfig.customOpenAIHasApiKey || false;

    if (modelConfig.provider === 'custom-openai') {
      try {
        const saved = await invoke<CustomOpenAIConfig>('api_save_custom_openai_config', {
        ...
        customOpenAIHasApiKey = saved.hasApiKey;
        hasApiKey = saved.hasApiKey;
        apiKeyConfigured = {
          ...apiKeyConfigured,
          customOpenai: saved.hasApiKey,
        };
        setCustomOpenAIApiKey('');
      } catch {
        console.error('Failed to save custom OpenAI configuration.');
        toast.error('无法保存自定义模型配置，请检查地址和参数。');
        return;
      }
    } else if (requiresApiKey && apiKey.trim()) {
      try {
        const result = await invoke<SummaryApiKeyMutationResult>('api_set_summary_api_key', {
          provider: modelConfig.provider as SummaryKeyProvider,
          apiKey: apiKey.trim(),
        });
        ...
        console.error('Failed to replace summary API key.');
        toast.error('无法保存 API 密钥，请检查后重试。');
        return;
      }
    }
    ...
    const updatedConfig: ModelConfig = {
      ...
      customOpenAIHasApiKey: modelConfig.provider === 'custom-openai' ? customOpenAIHasApiKey : false,
  ...
  const deleteCurrentApiKey = async () => {
    ...
    if (!['openai', 'claude', 'groq', 'openrouter', 'custom-openai'].includes(provider)) {
      return;
    }
    try {
      const result = await invoke<SummaryApiKeyMutationResult>('api_delete_summary_api_key', {
        provider: provider as SummaryKeyProvider,
      });
      setApiKey('');
      setCustomOpenAIApiKey('');
      setModelConfig((previous: ModelConfig) => ({
        ...
        hasApiKey: false,
        apiKeyConfigured: result.apiKeyConfigured,
        customOpenAIHasApiKey: provider === 'custom-openai'
          ? false
          : previous.customOpenAIHasApiKey,
      }));
      toast.success('已删除保存的 API 密钥。');
    } catch {
      toast.error('无法删除 API 密钥，请稍后重试。');
    }
  };
  ...
  const testCustomOpenAIConnection = async () => {
    ...
    try {
      const result = await invoke<SummaryConnectionTestResult>('api_test_custom_openai_connection', {
        endpoint: customOpenAIEndpoint.trim(),
        apiKey: customOpenAIApiKey.trim() || null,
        model: customOpenAIModel.trim(),
        useStoredKey: !customOpenAIApiKey.trim() && selectedProviderHasSavedKey,
      });
      if (result.ok) {
            ...
            <Select
              ...
              onValueChange={(value) => {
                ...
                setModelConfig({
                  ...
                  provider,
                  model,
                  hasApiKey: configuredKeyForProvider(provider),
                });
                setApiKey('');
                setCustomOpenAIApiKey('');
                  ...
                  invoke<any>('api_get_custom_openai_config').then((config) => {
                    if (config) {
                      ...
                      setModelConfig((previous: ModelConfig) => ({
                        ...
                        customOpenAIHasApiKey: !!config.hasApiKey,
                        apiKeyConfigured: {
                          ...previous.apiKeyConfigured,
                          customOpenai: !!config.hasApiKey,
                        },
                      }));
                    }
                  }).catch((err) => {
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                {selectedProviderHasSavedKey && (
                  <Button type="button" variant="ghost" size="sm" onClick={deleteCurrentApiKey}>
                    删除已保存密钥
                  </Button>
                )}
              </div>
              <Input
                ...
                placeholder={selectedProviderHasSavedKey ? '已配置；输入新密钥可替换' : '不需要认证时可留空'}
                className="mt-1"
              />
              <p className="mt-1 text-xs text-muted-foreground">
                {selectedProviderHasSavedKey ? '已配置密钥。留空会保留原密钥，应用不会回显。' : '尚未配置密钥。'}
              </p>
            </div>
          ...
          <div>
            ...
            <div className="relative mt-1">
              <Input
                ...
                placeholder={selectedProviderHasSavedKey ? '已配置；输入新密钥可替换' : '请输入 API 密钥'}
                className="pr-12"
              />
