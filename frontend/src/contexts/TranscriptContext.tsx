...
interface TranscriptContextType {
  ...
  transcriptEventsRef: MutableRefObject<TranscriptUpdate[]>;
...
export function TranscriptProvider({ children }: { children: ReactNode }) {
  ...
  const currentMeetingIdRef = useRef<string | null>(
  ...
  useEffect(() => {
    ...
    const setupRecordingListeners = async () => {
      try {
        ...
        // A page reload does not produce another recording-started event. Keep
        // using the recovery meeting ID immediately so subsequent live events
        // continue to be durable.
        const recoveredMeetingId = sessionStorage.getItem('indexeddb_current_meeting_id');
        if (recoveredMeetingId) {
          currentMeetingIdRef.current = recoveredMeetingId;
          setCurrentMeetingId(recoveredMeetingId);
        }
  ...
  // Revision-aware live transcript listener. Canonical events update the
  // current utterance in place; legacy events continue to append.
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        ...
        unlistenFn = await transcriptService.onTranscriptUpdate((update) => {
          ...
          const nextHistory = upsertTranscriptRevisionHistory(
            previousHistory,
            update,
          );
          ...
          setTranscripts(() => {
            const next = upsertTranscripts([], nextHistory);
    ...
    const syncFromBackend = async () => {
      ...
      if (recordingState.isRecording && transcripts.length === 0) {
        try {
          ...
          console.log(`[Reload Sync] Retrieved ${history.length} transcript events from backend`);

          const historyUpdates: TranscriptUpdate[] = history.map((segment) => ({
            ...segment,
            text: segment.text,
            timestamp: segment.timestamp ?? segment.display_time,
            source: segment.source ?? segment.audio_source ?? 'Audio',
            sequence_id: segment.sequence_id ?? 0,
            chunk_start_time: segment.chunk_start_time ?? segment.audio_start_time ?? 0,
            is_partial: segment.is_partial ?? false,
            confidence: segment.confidence ?? 0,
            audio_start_time: segment.audio_start_time ?? 0,
            audio_end_time: segment.audio_end_time ?? segment.audio_start_time ?? 0,
            duration: segment.duration ?? 0,
          }));
          const meetingId = currentMeetingIdRef.current;
          const indexedDbUpdates = meetingId
            ? await indexedDBService.getTranscriptEventUpdates(meetingId)
            : [];
          // Events may arrive while the backend/IndexedDB reads are awaiting.
          // Merge the live ref last in this same synchronous turn so a fresh
          // final cannot be overwritten by the older reload snapshot.
          const mergedHistory = mergeTranscriptRevisionHistories(
            indexedDbUpdates,
            historyUpdates,
            transcriptEventsRef.current,
          );
          const formattedTranscripts = upsertTranscripts([], mergedHistory);
