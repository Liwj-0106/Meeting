...
import { transcriptUpdateToTranscript } from '@/lib/transcript-events';
import {
      ...
      try {
        ...
        unlistenFn = await listen<{
          ...
          recordingStoppedDataRef.current = (async () => {
            ...
            if (folder_path) {
              try {
                const ticket = await liveSummaryService.issueRecordingBindingTicket(folder_path);
                liveSummaryService.rememberRecordingBindingTicket(ticket);
              } catch (error) {
                // The start-time ticket remains usable. This exact-folder
                // refresh mainly restores a renderer that was reloaded during
                // recording and must never block transcript persistence.
                console.warn('Live summary binding ticket was not refreshed after stop', {
  ...
  const handleRecordingStop = useCallback(async (isCallApi: boolean) => {
    ...
    try {
      ...
      if (isCallApi && transcriptionComplete == true) {
        ...
        const revisionHistory = transcriptEventsRef.current.map((event, index) => (
          transcriptUpdateToTranscript(event, {
            legacyId: `legacy-save:${index}:${event.sequence_id}`,
          })
        ));
        const transcriptsToPersist = revisionHistory.length > 0
          ? revisionHistory
          : freshTranscripts;
        ...
        console.log('💾 Saving COMPLETE transcripts to database...', {
          ...
          total_characters: freshTranscripts.reduce((total, transcript) => total + transcript.text.length, 0),
        });

        try {
          const summaryTicket = liveSummaryService.getRecordingBindingTicket();
          const responseData = await storageService.saveMeeting(
            savedMeetingName || meetingTitle || 'New Meeting',  // PREFER savedMeetingName (backend source)
            transcriptsToPersist,
            folderPath,
            summaryTicket?.handle ?? null,
          );
          ...
          if (summaryTicket) {
            const summaryBinding = responseData.summary_binding;
            if (
              summaryBinding?.status === 'bound'
              || summaryBinding?.status === 'rejected'
            ) {
              liveSummaryService.clearRecordingBindingTicket(summaryTicket.handle);
            }
            if (summaryBinding?.status === 'pending_retry') {
              toast.warning('会议已保存，实时总结等待重试', {
                description: '逐字稿和录音不受影响；请检查总结模型配置。',
              });
            } else if (summaryBinding?.status === 'rejected') {
              toast.warning('会议已保存，实时总结未绑定', {
                description: summaryBinding.error?.message
                  ?? '绑定凭据已失效或与本次录音不匹配。',
              });
            }
          }
