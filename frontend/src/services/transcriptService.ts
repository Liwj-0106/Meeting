...
/** Exact recovery DTO serialized by Rust RecordingSaver. */
export type TranscriptHistoryItem = Partial<TranscriptUpdate> & {
  text: string;
  display_time: string;
  sequence_id: number;
};
...
export class TranscriptService {
   ...
   * @returns every revision currently held by the recording backend
   */
  async getTranscriptHistory(): Promise<TranscriptHistoryItem[]> {
    return invoke<TranscriptHistoryItem[]>('get_transcript_history');
  }
