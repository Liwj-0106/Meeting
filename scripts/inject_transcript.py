...
def validate_db_path(path: Path) -> Path:
    """Resolve a database path without allowing this helper to write on C:."""
    expanded = path.expanduser()
    raw = str(expanded).replace('/', '\\')

    if os.name == "nt" and (raw.startswith('\\\\.\\') or raw.startswith('\\\\?\\')):
        raise ValueError("Windows device and volume namespace paths are not supported")

    if not expanded.is_absolute():
        expanded = Path.cwd() / expanded

    # strict=False resolves every existing prefix, including directory
    # junctions, while still permitting a not-yet-created final component.
    resolved = expanded.resolve(strict=False)
    if os.name == "nt" and resolved.drive.rstrip(':').casefold() == 'c':
        raise ValueError("Meetily database helpers must not write to the C drive")
    return resolved


def get_default_db_path() -> Path:
    """Get the repository-owned portable database path."""
    configured_data_dir = os.environ.get("MEETILY_DATA_DIR", "").strip()
    if configured_data_dir:
        return validate_db_path(Path(configured_data_dir) / "meeting_minutes.sqlite")

    # A checkout with a staged portable runtime owns its writable database.
    # Prefer that location even when this helper is launched directly instead
    # of through the portable command wrapper, so it cannot silently create a
    # second Meetily database under the Windows user profile.
    portable_data_dir = Path(__file__).resolve().parent.parent / "portable" / "app-data"
    return validate_db_path(portable_data_dir / "meeting_minutes.sqlite")
...
Example usage:
  ...
  python inject_transcript.py --csv data.csv --db ~/custom/path.sqlite
    ...
    parser.add_argument(
        '--db', '-d',
        default=None,
        help='Database path (defaults to repository portable/app-data; C-drive paths are rejected)'
    )
    ...
    try:
        db_path = validate_db_path(Path(args.db)) if args.db else get_default_db_path()
    except ValueError as error:
        print(f"Error: Unsafe database path: {error}", file=sys.stderr)
        sys.exit(1)
